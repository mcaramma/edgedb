...
.>(test::name)[IS std::str]": {
    ...
    def test_edgeql_ir_scope_tree_14(self):
        # NOTE: "ns~1@@Card" is a redundant entry, generated by
        # evaluating the schema-level computable in a detached
        # namespace.  The rptr of Card.<deck actually points to
        # an unnamespaced (test::Card) via view prefix mapping.
        """
        WITH MODULE test
        SELECT Card.owners
...
% OK %
        "FENCE": {
            "(test::Card).>(test::owners)[IS test::User]": {
                "(test::Card)",
                "FENCE": {
                    "ns~1@@(test::Card).<(test::deck)[IS test::User]": {
                        "ns~1@@(test::Card)"
                    }
                }
            }
